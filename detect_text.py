...
def detect(orig, model_file, min_confidence=0.5):
    image = orig.copy()
    ...
    for (startX, startY, endX, endY) in boxes:
        ...
        # ratios
        startX = max(int(startX * rW) - d, 0)
        startY = max(int(startY * rH) - d, 0)
        endX = min(int(endX * rW) + d, W-1)
        endY = min(int(endY * rH) + d, H-1)
        #print(startX, startY, endX, endY)
        # draw the bounding box on the image
        img = gray[startY:endY, startX:endX]
